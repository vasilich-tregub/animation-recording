...
The background image is generated from a base64 encoded image data URL; the DataURL const is defined in text-renderer.js 
and the image is loaded within arrow function of the window.onload event listener. The base64-encoded data of this 
DataURL const is generated elsewhere from the image Earth512x512.jpeg included with this repo.
...
an http server at all cost: 
[file:/// URL origin is *opaque*](https://developer.mozilla.org/en-US/docs/Web/Security/Defenses/Same-origin_policy#file_origins), 
and you may trigger CORS errors when, for example, loading texture from image file resources -- without using my dataURL 
workaround. So, a bonus with this exercise is that it shows how some components can be developed without an http server.
